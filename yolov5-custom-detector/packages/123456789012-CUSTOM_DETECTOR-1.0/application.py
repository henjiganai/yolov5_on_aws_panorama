...
class Application(panoramasdk.node):
    ...
    def __init__(self):
        ...
        self.MODEL_WITDH = 640
        self.MODEL_DIM = (self.MODEL_WITDH, self.MODEL_WITDH)
        self.frame_num = 0
    ...
    def process_media(self, stream):
        ...
        inference_results = self.call({"images":image_data}, self.MODEL_NODE)
         # Log metrics
    ...
    def process_results(self, inference_results, stream):
        ...
        logger.debug('Output one type: {}'.format(type(output)))
        boxes = non_max_suppression(torch.tensor(output), self.threshold, self.iou_thres)

        for count, box in enumerate(boxes):
            logger.debug(f'Result shape({count}): {box}')

            for i, det in enumerate(box):
                if len(det) == 0:
                    continue

                annotator = Annotator(stream.image, line_width=self.line_thickness, example=str(self.classes))
                ...
                scale = media_scale / np.array([self.MODEL_WITDH, self.MODEL_WITDH, self.MODEL_WITDH, self.MODEL_WITDH])
                media_xyxy = xyxy[: 4] * scale
                annotator.box_label(media_xyxy, label, color=colors(c, True))
    ...
    def put_metric_data(self, metric_name, metric_value):
        ...
        dimension_value = 'yolov5-custom-detector'
        try:
